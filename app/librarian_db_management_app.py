...
from config_app import ALGORITHM, ACCESS_TOKEN_EXPIRE_WEEKS, SECRET_KEY
...
def login(form_data: OAuth2PasswordRequestForm = Depends(), db: Session = Depends(get_db)):
    user = get_user(db, form_data.username)  # OAuth2PasswordRequestForm использует username вместо email
    if not user:
        raise HTTPException(
