...
from app.init_db_app import ensure_db_exists, add_first_librarian_if_needed  # замените your_module на имя вашего модуля
...
def test_add_first_librarian_if_needed_creates_user(temp_db_path):
    ...
    session = Session()
    user = session.query(User).filter_by(email="first_librarian@library.com").first()
    session.close()
...
def test_add_first_librarian_if_needed_skips_if_users_exist(temp_db_path):
    ...
    Session = sessionmaker(bind=engine)
    session = Session()
    user = User(email="existing@library.com", password_hash=bcrypt.hash("password"))
    session.add(user)
    session.commit()
